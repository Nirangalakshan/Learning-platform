...
export default function Quizzes() {
  ...
  const handleCopyQuiz = () => {
    ...
    const quizText = generatedQuiz.questions
  ...
  const handleDownloadQuiz = () => {
    if (!generatedQuiz) return;
    const quizContent = `${generatedQuiz.subject} - ${
      generatedQuiz.questionType
    } Quiz
...
${q.options ? q.options.map((opt) => `  - ${opt}`).join("\n") : ""}`
  ...
  const availableLessons = subject
    ? SUBJECTS_WITH_TOPICS[subject as keyof typeof SUBJECTS_WITH_TOPICS]
    : [];

  return (
    <div className="space-y-6">
      {/* Generator Card */}
      <Card className="glass border-border/50 rounded-2xl p-4 w-[98%] mx-auto my-4">
        <CardHeader>
          ...
          <CardDescription>
            Create customized quizzes using AI based on subject, lessons, and
            question type
          </CardDescription>
        </CardHeader>
            ...
            <div className="space-y-3 p-4 bg-background/50 rounded-lg border border-border/50">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <Checkbox
                    id="all-syllabus"
                    checked={isAllSyllabus}
                    onCheckedChange={handleAllSyllabusToggle}
                  />
                  <label
                    htmlFor="all-syllabus"
              ...
              {isAllSyllabus && (
                <p className="text-xs text-muted-foreground italic">
                  Quiz will cover all lessons from {subject}
                </p>
              )}
            </div>
          )}
      ...
      {savedQuizzes.length > 0 && (
        <Card className="glass border-border/50 rounded-2xl w-[98%] mx-auto">
          <CardHeader>
            ...
            <CardDescription>
              View and manage your AI-generated quizzes
            </CardDescription>
          </CardHeader>
              ...
              >
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {quiz.questionCount} {quiz.questionType} questions •{" "}
                    {quiz.difficulty}
                  </p>
                  <p className="text-xs text-primary/80 mt-1">
                    {quiz.language.charAt(0).toUpperCase() +
                      quiz.language.slice(1)}{" "}
                    •{" "}
                    {quiz.isAllSyllabus
                      ? "All Syllabus"
                      : quiz.lessons.join(", ")}
