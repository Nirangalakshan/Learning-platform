...
export async function POST(request: NextRequest) {
  try {
    ...
    const languageInstruction =
      language === "sinhala"
        ? "Generate the quiz in Sinhala language."
        : "Generate the quiz in English language.";

    const prompt = `Generate ${questionCount} ${difficulty.toLowerCase()} level ${questionType.replace(
      "-",
      " "
    )} questions for ${subject} subject (Sri Lankan A/L Science).
...
Return the response as JSON with this structure:
{
  "questions": [
    ...
    try {
      const responseText =
        aiData.message || aiData.response || JSON.stringify(aiData);
